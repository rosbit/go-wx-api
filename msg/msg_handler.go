...
var messageHandlers = map[string]FnMessageHandler {
...
var eventHandlers = map[string]FnMessageHandler {
...
func handleReceivedMessage(receivedMsg ReceivedMsg, msgType string) ReplyMsg {
	if fn, ok := messageHandlers[msgType]; ok {
		return fn(receivedMsg)
	}
	return nil
}

func handleReceivedEvent(receivedMsg ReceivedMsg, eventType string) ReplyMsg {
	if fn, ok := eventHandlers[eventType]; ok {
		return fn(receivedMsg)
	}
	return nil
}
